...
ActiveRecord::Base.transaction do
  ...
  kenobe = User.create(username: "Ben Kenobe", biography: "The force is strong", picture_url: "http://images4.fanpop.com/image/photos/24000000/Ben-Kenobi-ben-kenobi-24018429-447-335.jpg", password: "starwars")
	...
	data = User.create(username: "Commander Data", picture_url: "http://static.comicvine.com/uploads/original/10/103184/3143248-lt-commander-data-star-trek-the-next-generation-9406567-1664-2560.jpg", biography: "My neuro positronic network enables me to calculate pi to a very precise value. I am interested in comedy and will practice my skills at a local comedy club.", password: "wookies")
...
# location_pictures.create([	])
  ...
  att = wookies.locations.create(title: "AT&T Ballpark", description: "Home of your San Francisco Giants. Winners of the 2010, 2012, and 2014 World Series. Go Giants!", address: "24 Willie Mays Plaza, San Francisco, CA", price: 400)

  att.reviews.create(user_id: kirk.id, rating: 5, body: "These hot dogs behind first base are quite tasty.")

  att.reviews.create(user_id: data.id, rating: 3, body: "Sir, isn't there a better way to utilize the players on each team.")

  att.reviews.create(user_id: kenobe.id, rating: 4, body: "The force is strong here. I really enjoyed the bleachers.")

  att.reviews.create(user_id: worf.id, rating: 2, body: "Klingons would use the bats as weapons and drink blood wine afterwards!")

  att.reviews.create(user_id: quark.id, rating: 2, body: "Humons. The markup for goods is not as high as it could be. They're losing quite the profit margin.")

  att.reviews.create(user_id: borg.id, rating: 5, body: "You will be assimilated.")
  ...
  beldenplace.reviews.create(user_id: daniel.id, rating:5, body: "This place truly is a hidden gem. Five stars.")
